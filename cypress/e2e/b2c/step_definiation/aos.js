...
const paymentContinue = () => {
  ...
  cy.get('body').then(($body) => {
    ...
    if ($body.find("#terms_cond_checkbox").length > 0) {
      ...
      cy.log('Check box is not founded.');
    }
  });
...
Then("I need to validate Refundable Option", () => {
  const refundFilter = String(bookingData[Cypress.env("dataIndex")]["RefundableOption"]);
  cy.get("#dropdownRefundable").should("be.visible").click();
  cy.get(".dropdown-menu.show")
    .contains(refundFilter)
    .click({ force: true });
...
const travelerPagePaymentContinue = () => {
  cy.get('body').then(($body) => {
      ...
      cy.get('.empireF_amountWrapper > h4') // Get the original total amount
        ...
        .then((text) => {
          ...
          cy.get('.empireF_amountWrapper > h4') // Validate new total
            .invoke('text')
            .then((newText) => {
              const newTotal = parseFloat(newText.replace(/[^\d.-]/g, ''));
  ...
  cy.get(".fareOptionCardMobile > .empireFlight_fareOptCardsHead > .FareoptionselectedFlight").then(($flightCards) => {
    ...
    cy.wrap($flightCards).each(($flightCard, index) => {
      ...
      cy.get("@flightCard")
        ...
        .invoke("text")
        .then((flightName) => {
          const cleanedFlightName = flightName.trim();
          cy.log(`Flight Name: ${cleanedFlightName}`);
          cy.softAssert(() => {
      ...
      cy.get("@flightCard")
        ...
        .then((startTime) => {
          ...
          cy.log(`Flight Start Time: ${cleanedStartTime}`);
          cy.softAssert(() => {
      ...
      cy.get("@flightCard")
        ...
        .then((destination) => {
          ...
          cy.log(`Destination: ${cleanedDestination}`);
          cy.softAssert(() => {
      ...
      cy.get("@flightCard")
        ...
        .invoke("text")
        .then((fareOption) => {
          const cleanedFareOption = fareOption.trim();
          cy.log(`Fare Option: ${cleanedFareOption}`);
          cy.softAssert(() => {
