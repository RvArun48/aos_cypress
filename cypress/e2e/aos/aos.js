...
Then("I need to add the traveller details for {string}", (passengerType) => {
  ...
  dataSet.forEach((rowData, index) => {
    ...
    if (index >= 0 && index < dataLimit) {
      ...
      // Function to handle filling out form fields if they exist
      const documentType = rowData["Document Type"]; // This should be one of "Passport", "Local ID", "Iquma ID", or "National ID"

      // Function to handle filling out form fields if they exist
      function fillDocumentInformation(selector, value, index) {
        if (value) {
          ...
          cy.get(selector)
            ...
            .then(($element) => {
              if ($element.is(":visible")) {
                cy.wrap($element)
                  .click()
                  .clear() // Clear any existing value if needed
                  .type(value)
                  .should("have.value", value);
              } else {
                cy.log(`Field ${selector} is not visible. Skipping...`);
              }
            });
        }
      }

      // Check if the fields are available and the document type is "Passport"
      cy.get("body").then(($body) => {
        const i = 0; // Assuming 'i' is the index you're using for each element

        if (documentType === "Passport") {
          if (
            $body.find("[formcontrolname='DocumentIssuingCountry']").length > 0
          ) {
            // Fill in the Passport information
            fillDocumentInformation(
              "[formcontrolname='DocumentIssuingCountry']",
              rowData["Issuing Country"],
              i
            );
            fillDocumentInformation(
            ...
            fillDocumentInformation(
              "[formcontrolname='DocumentIssueDay']",
              rowData["PID Date"],
              i
            );
            fillDocumentInformation(
              "[formcontrolname='DocumentIssueMonth']",
              rowData["PID Month"],
              i
            );
            fillDocumentInformation(
              "[formcontrolname='DocumentIssueYear']",
              rowData["PID Year"],
              i
            );
            fillDocumentInformation(
              "[formcontrolname='DocumentExpiryDay']",
              rowData["PED Date"],
              i
            );
            fillDocumentInformation(
              "[formcontrolname='DocumentExpiryMonth']",
              rowData["PED Month"],
              i
            );
            fillDocumentInformation(
              "[formcontrolname='DocumentExpiryYear']",
              rowData["PED Year"],
              i
            );
          } else {
            cy.log(
              "Passport information fields are not present. Skipping this section."
            );
          }
          // } else if (documentType === 'Local ID') {
          //   // Fill in Local ID information if fields are present
          //   fillDocumentInformation("[formcontrolname='LocalIDNumber']", rowData['Local ID Number'], i);
          //   fillDocumentInformation("[formcontrolname='LocalIDIssueDate']", rowData['Local Issue Date'], i);
          //   fillDocumentInformation("[formcontrolname='LocalIDExpiryDate']", rowData['Local Expiry Date'], i);
          // } else if (documentType === 'Iquma ID') {
          //   // Fill in Iquma ID information if fields are present
          //   fillDocumentInformation("[formcontrolname='IqumaIDNumber']", rowData['Iquma ID Number'], i);
          //   fillDocumentInformation("[formcontrolname='IqumaIDIssueDate']", rowData['Iquma Issue Date'], i);
          //   fillDocumentInformation("[formcontrolname='IqumaIDExpiryDate']", rowData['Iquma Expiry Date'], i);
          // } else if (documentType === 'National ID') {
          //   // Fill in National ID information if fields are present
          //   fillDocumentInformation("[formcontrolname='NationalIDNumber']", rowData['National ID Number'], i);
          //   fillDocumentInformation("[formcontrolname='NationalIDIssueDate']", rowData['National Issue Date'], i);
          //   fillDocumentInformation("[formcontrolname='NationalIDExpiryDate']", rowData['National Expiry Date'], i);
          // } else {
          //   cy.log(`Unknown document type: ${documentType}. Skipping document information entry.`);
        }
      });
...
Then("I need to click continue to payment", () => {
  cy.contains("Payfort English").click();
...
Then("I need to validate the time", () => {
  ...
  cy.wait(500); // Wait 500ms for the UI to update (adjust based on your app's performance)
...
Then("I need to validate the stop", () => {
  // Get the selected stop type from the booking data
  const selectedStopType = bookingData[Cypress.env("dataIndex")]["Stop"];

  if (selectedStopType) {
    // Check if stop type is available
    // Ensure dropdown is visible and apply the selected filter
    ...
    cy.wait(2000); // Wait for the flight results to update
...
Then("I need to validate the price", () => {
  ...
  const resetFilterButtonLocator = 'button:contains("Reset All Filters")'; // Using the proper locator for the Reset button

// Check if the reset filter button exists
cy.get(resetFilterButtonLocator).then((buttons) => {
  if (buttons.length > 0) {
    cy.log(`Found ${buttons.length} Reset All Filters button(s), checking visibility.`);

    // Check if the button is visible
    cy.get(resetFilterButtonLocator).should('be.visible').click({ force: true });
...
Then("I need to validate flight card", () => {
  ...
  const flightLimit = 2; // Adjust this limit as needed
...
Then("I need to validate Duration", () => {
  ...
  Cypress.Promise.try(() => {
    ...
    cy.log("Skipped this block due to a failure.");
  });
...
Then("I click on the fare breakup", () => {
  cy.contains("Fare Breakup").click({ force: true });
});
...
Then("I click on the baggage", () => {
  cy.contains(" Baggage ").click({ force: true });
});
